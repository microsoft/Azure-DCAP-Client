...
static void GetCertsTest()
{
    ...
    constexpr sgx_cpu_svn_t CPU_SVN_MAPPED = {0x05,
                                              0x05,
                                              0x02,
                                              0x04,
                                              0x01,
...
static void GetCrlTest()
{
    ...
    static constexpr uint8_t TEST_FMSPC[] = {
        0x00, 0x90, 0x6E, 0xA1, 0x00, 0x00};
...
extern void QuoteProvTests()
{
    ...
    // call is fast enough
    assert(
        fabs(duration_curl - duration_local) > 0.0001 &&
        duration_local < 0.001);
...
extern void QuoteProvTests()
{
    ...
    // First pass: Get the data from the service, no cache allowed
    //
    _putenv("AZDCAP_BASE_CERT_URL=https://global.acccache.azure.net/sgx/"
            "certificates");

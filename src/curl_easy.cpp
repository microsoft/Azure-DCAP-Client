    ...
    const std::string& ca_cert)
{
    ...
    if (CURLE_OK != curl_easy_setopt(easy->handle, CURLOPT_URL, url.c_str()))
    {
        // Accoring to CURL docs, the only error that can occur is OOM
        throw std::bad_alloc();
    }

    // According to libcurl docs, the rest of the calls should never fail.
    CURLcode result =
        curl_easy_setopt(easy->handle, CURLOPT_WRITEFUNCTION, &write_callback);
    assert(result == CURLE_OK);
    throw_on_error(result, "curl_easy_setopt(CURLOPT_WRITEFUNCTION)");

    result = curl_easy_setopt(easy->handle, CURLOPT_WRITEDATA, easy.get());
    assert(result == CURLE_OK);
    throw_on_error(result, "curl_easy_setopt(CURLOPT_WRITEDATA)");

    result = curl_easy_setopt(
        easy->handle, CURLOPT_HEADERFUNCTION, &header_callback);
    assert(result == CURLE_OK);
    throw_on_error(result, "curl_easy_setopt(CURLOPT_HEADERFUNCTION)");

    result = curl_easy_setopt(easy->handle, CURLOPT_HEADERDATA, easy.get());
    assert(result == CURLE_OK);
    throw_on_error(result, "curl_easy_setopt(CURLOPT_HEADERDATA)");

    result = curl_easy_setopt(easy->handle, CURLOPT_FAILONERROR, 1L);
    assert(result == CURLE_OK);
    throw_on_error(result, "curl_easy_setopt(CURLOPT_FAILONERROR)");
    ...
    if (!ca_cert.empty())
    {
        ...
        result = curl_easy_setopt(
            easy->handle, CURLOPT_SSL_CTX_FUNCTION, ssl_context_callback);
        assert(result == CURLE_OK);
        throw_on_error(result, "curl_easy_setopt(CURLOPT_FAILONERROR)");

        result =
            curl_easy_setopt(easy->handle, CURLOPT_SSL_CTX_DATA, easy.get());
        assert(result == CURLE_OK);
        throw_on_error(result, "curl_easy_setopt(CURLOPT_FAILONERROR)");
    }
